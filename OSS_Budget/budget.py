...
class Budget:
    ...
    def edit_expenses(self, index):
        ...
        new_category = input("카테고리 (예: 식비, 교통 등): ")
        if new_category != "-1":
            origin.category = new_category

        new_description = input("설명: ")
        if new_description != "-1":
            origin.description = new_description

        new_amount = input("금액(원): ")
        if new_amount != "-1":
            origin.amount = new_amount
            print("수정되었습니다.")
            return
    ...
    def list_expenses(self):
        ...
        while True:
            ...
            editYN = input("수정하려면 \'E\', 메뉴로 돌아가려면 아무 키나 눌러주세요>")
            if editYN == "E":
                num = int(input("수정할 지출목록의 번호를 입력하세요"))
